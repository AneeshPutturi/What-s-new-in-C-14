...
2. **Compare the Code**: Navigate to the `Features` folder to compare `*.Before.cs` (traditional) and `*.After.cs` (modern) implementations.

## 📂 Project Structure
* **Program.cs**: Orchestrates the live demonstrations.
* **Features/**: Subdivided by feature (e.g., Primary Constructors, Collection Expressions).
  * `*.Before.cs`: The traditional pre-modern approach.
  * `*.After.cs`: The clean, modern approach.
  * `*.Demo.cs`: Logic for the console display.

## 🛠 Features Covered
* **Primary Constructors**: Capture state directly in the class signature.
* **Collection Expressions**: Simplified `[]` syntax for lists and arrays.
